<?php
/**
 * Stitch product detail template part for AllScented.
 * If $stitch_product_id is provided, it renders that product directly.
 * Otherwise it follows the page template resolution: ?product=ID, then page ACF.
 */

defined('ABSPATH') || exit;

$product_id = 0;
$stitch_args_product_id = (isset($args['stitch_product_id']) && is_numeric($args['stitch_product_id'])) ? (int) $args['stitch_product_id'] : 0;
if ($stitch_args_product_id > 0) {
    $product_id = $stitch_args_product_id;
} elseif (isset($stitch_product_id) && (int) $stitch_product_id > 0) {
    $product_id = (int) $stitch_product_id;
} else {
...
if (!$product) {
    $empty_message = (isset($args['context']) && $args['context'] === 'product')
        ? 'This product is unavailable.'
        : 'Please select a product in the page settings, or append ?product=ID to the URL.';
